...
namespace ClothingForDocuSign.Console.UnitTests.Views
{
	...
	public class ClothingConsoleViewTests
	{
		...
		public void Can_CommandEventListener_Add_Into_Final_Result()
		{
			...
			clothingCommandMock.Setup(m => m.Response(It.IsAny<TemperatureType>())).Returns("Test");
		...
		private void Check_Input(string input, string expectedOutput, TemperatureType? expectedTempratureType, string expectedCommandIdStr)
		{
			...
			TemperatureType? tempratureTypeActual = null;
			IList<int> commandIdActual = null;

			var clothingControllerMock = new Mock<IClothingController>();
			clothingControllerMock.Setup(m =>
						m.OnInputFinished(It.IsAny<TemperatureType?>(), It.IsAny<IList<int>>(), false))
						.Callback<TemperatureType?, IList<int>, bool>((tempratureType, commandIds, exit) =>
						{
