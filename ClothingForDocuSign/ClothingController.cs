...
namespace ClothingForDocuSign.ConsoleView
{
	public class ClothingController : IClothingController
	{
		...
		public void OnInputFinished(TemperatureType? tempratureType, IList<int> commandIds, bool exit = false)
		{
			if (exit) return;
		...
		private IClothingModel GetClothingModel(TemperatureType tempratureType)
		{
			var model = _clothingColdModel;

			if (tempratureType == TemperatureType.HOT)	model = _clothingHotModel;
